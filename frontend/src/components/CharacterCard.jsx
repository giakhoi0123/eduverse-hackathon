...
function CharacterCard({ character, onSelect }) {
  ...
  return (
    <div
      ...
      style={{
        boxShadow: isHovered
          ? `0 20px 50px -12px ${getGlowColor(character.color)}, 0 0 0 3px ${
              character.color
            }20`
          : undefined,
      }}
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to bottom right, ${character.color}, transparent)`,
        }}
      ></div>
      ...
      {/* Avatar Section */}
      <div
        className={`relative w-full h-52 bg-gradient-to-br ${getColorClass()} rounded-xl mb-4 flex flex-col items-center justify-center text-white overflow-hidden group-hover:shadow-2xl transition-all duration-300`}
      >
        {/* Character Image */}
        {character.avatar ? (
          <img
        ...
        {character.voicePreview && (
          <div className="relative z-10 flex items-center space-x-1.5 bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-full transform group-hover:scale-110 transition-all duration-300 mb-2">
      ...
      <div className="relative z-10">
        <h3
          className="font-bold text-xl mb-1.5 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:bg-clip-text transition-all duration-300"
          style={{
            backgroundImage: isHovered
              ? `linear-gradient(to right, ${character.color}, ${character.color}dd)`
              : undefined,
          }}
        >
          {character.name}
